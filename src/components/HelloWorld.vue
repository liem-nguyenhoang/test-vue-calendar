    ...
    <v-col>
      <v-sheet height="600">
        <v-calendar
          ...
          color="primary"
          type="month"
          display-value="2025-01-20"
          :event-overlap-mode="'stack'"
          show-adjacent-months  hide-week-number
...
<div class="v-calendar-month__days  days__7"  >
  <template v-for="week in chunkArray(daysList, 7)"  >
    <template v-for="day in week"  >
     <VCalendarMonthDay    :day="day" :title="getTitle(day)" :color="getColor(day)" :disabled="getMode(day)"
...
return events?.value.filter(e => adapter.isSameDay(day.date, e.start) || adapter.isSameDay(day.date, e.end))
}

function getAllowedDates(day){
console.log('adapter getMonth',adapter.getMonth(adapter.date('2025-03-12')))
console.log('adapter getWeekArray',adapter.getWeekArray(adapter.getMonth(adapter.date('2025-03-12')),1))
